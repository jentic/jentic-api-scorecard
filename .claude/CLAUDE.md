...
- **`action.yml` + `action/`** — the Marketplace-listable composite GitHub Action (Phase 19). `action.yml` at the repo root (Marketplace requires the manifest at root) wraps `npx @jentic/api-scorecard-cli@<cli-version>` for one `score --format json --detail diagnostics -o report.json` pass, then runs `action/postprocess.mjs` to derive SARIF, the HTML scorecard, and the Markdown run summary from that single capture (score-once, format-many — each formatter is a pure function of the result). The helper computes the gate (`min-score` strict `<`, `max-errors`/`max-warnings` against the **full** diagnostics, not the severity-filtered SARIF), applies the `severity` filter then the `max-findings` cap (lowest-severity-first, logged), and writes `$GITHUB_STEP_SUMMARY`; the gate verdict goes to `$GITHUB_OUTPUT` and a final step fails the build **after** the publish steps so SARIF / HTML / summary land even on failure. The SARIF upload is guarded so a fork PR's read-only token skips it with a notice instead of hard-failing. The helper imports the CLI's `./sarif` / `./markdown` exports and formatter-html's `format()`; a setup step `npm install`s them adjacent to the helper unless already resolvable (the in-repo `.github/workflows/action-selftest.yml` builds the workspace first, so the helper resolves the local build's not-yet-published exports). `cli-version` defaults to the release that ships those exports (verify it matches the just-cut version at release time). Pre-bundling the helper is infeasible — HTML `format()` reads a sibling `dist/app/index.html` at runtime.
