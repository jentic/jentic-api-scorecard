...
- **CI**: `.github/workflows/ci.yml` runs `python-lint` + `python-test` (`cd docker && uv sync --frozen` → `uv run poe lint:ci` / `uv run poe test`), plus `typescript-lint` + `typescript-build` + `typescript-test` + `typescript-e2e` (`npm ci` → `npm run lint` / `npx lerna run build:typescript` / `npm test` / build CLI + buildx-cached image + `npx lerna run test:e2e`) on every PR. No path filters. The `typescript-e2e` job uses `docker/build-push-action@v7` with `cache-from: type=gha, cache-to: type=gha,mode=max` so unchanged docker layers are reused across PRs.
