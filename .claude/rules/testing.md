...
- **CI**: `.github/workflows/ci.yml` runs `python-lint` + `python-test` (`cd docker && uv sync --frozen` → `uv run poe lint:ci` / `uv run poe test`), plus `typescript-lint` + `typescript-build` + `typescript-test` + `typescript-e2e` (`npm ci` → `npm run lint` / `npx lerna run build:typescript` / `npm test` / `npm run build:image` + `npm run test:e2e`) on every PR. No path filters.
...
- Changed orchestration code in `packages/cli/src/` (`commands/score.ts`, `docker.ts`, `bundle.ts`, `index.ts`) or anything in `docker/src/` → also run e2e (`npm run test:e2e -w @jentic/api-scorecard-cli`). The script chains `build` (TypeScript + image at the matching tag) before mocha.
