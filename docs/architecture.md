...
| `--output` / `-o` `<file>` | stdout | Write report output to `<file>` instead of stdout. Useful for CI artifacts, Windows scripts, and future HTML/Markdown outputs where shell redirection is awkward. When set, spinner still goes to stderr. The file is written atomically (temp file + rename) so a partial write — out of disk, signalled exit — never replaces the previous file with a truncated one. |
