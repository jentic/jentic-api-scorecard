...
1. **Convert (Swagger 2.0 / Google Discovery → OpenAPI 3.0.0).** If the input is detected as Swagger 2.0 (`is_swagger_2`) or Google Discovery (`is_google_format`), the engine shells out to `npx --yes api-spec-converter --from=<fmt> --to=openapi_3 --syntax=json`. The output is always **OpenAPI 3.0.0** — `api-spec-converter`'s `openapi_3` target stamps `openapi: 3.0.0` and has no 3.1 mode — so a Swagger 2.0 author is scored against a machine-generated 3.0.0 document whose ceiling is set by 3.0.0 plus the converter's fidelity, never the 2.0 doc they actually ship. Inputs already at OpenAPI 3.0.x / 3.1.x pass through unconverted and are scored as authored. This conversion is **unconditional** — there is no `OASProcessConfiguration` flag to disable it — and emits an informational diagnostic (`imported-spec-is-swagger-2` / `imported-spec-is-google-discovery`); a conversion failure emits `failed-to-convert-swagger-2-to-oas3` and scoring proceeds on the original. The runner watches `result.diagnostics` for those two conversion codes and writes a one-line **stderr notice** (exit stays 0, scorecard still streamed) warning that diagnostic locations may not match the author's original file — because at the default `--detail dimensions` the informational diagnostic itself is filtered out of the report (see `version_guard.conversion_notice_message`, issue #113).
   ...
   **Supported OpenAPI versions: 3.0.x and 3.1.x only. OpenAPI 3.2 is _not_ supported.** The failure is **partial and uneven across validators**, which is what makes it dangerous rather than merely unsupported (all verified 2026-06-18):
      ...
      Because the zeroed count-signals have nothing to penalize, they score perfect and **drown out** the real Spectral/Redocly findings, so a 3.2 document scores *higher* than the same document at 3.0.x — a minimal 3.2.0 doc scored 92.99/A+ where the identical doc at 3.0.4 scored 70.19/A-. The score is inflated and meaningless. The scoring calculators list `(OPENAPI, "3.2.*")` in their `target_specs`, but SpecLynx underneath does not honor it. The runner therefore **rejects 3.2 outright** (`docker/src/jentic_scorecard_runner/version_guard.py`): stdin inputs are checked before scoring (from the buffered bytes), URL inputs after scoring (from the engine-emitted `apiMetadata.specificationVersion`, the first point the runner sees the version), and either path exits `SPEC_FAILURE` (5) with a guidance message instead of emitting the inflated number. Version *detection* delegates to the engine's canonical `jentic.apitools.openapi.common.version_detection` (`get_version` / `is_openapi_32`, shipped by `jentic-openapi-common`, pinned directly in `docker/pyproject.toml`) so there is one source of truth for parsing version strings; only the *policy* — that 3.2 is refused — lives in `version_guard.py`. Treating 3.2 as out of scope is part of the effective contract until SpecLynx adds 3.2 support; see issue #113.
...
| 5 | Spec-policy failure: an unsupported OpenAPI version. The runner rejects OpenAPI 3.2 (which the engine cannot parse and would score as inflated) — stdin inputs before scoring, URL inputs after, via `version_guard.py` (see §6). |
