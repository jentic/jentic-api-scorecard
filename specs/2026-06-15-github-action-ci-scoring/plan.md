...
6. Resolve how the helper accesses its two libraries at action runtime — `formatSarif` (CLI `./sarif` subpath) and the HTML `format()` (`@jentic/api-scorecard-formatter-html`). A composite action has no `node_modules` for them, so pick one: (a) an explicit setup step that `npm install`s both packages pinned to the action version, or (b) pre-bundle `postprocess.mjs` with its deps (esbuild/ncc) and commit the artifact. Document the choice in this plan once made; whichever is chosen, the resolved versions must match the action's pinned CLI version.
7. Create the Node helper `action/postprocess.mjs`: parse `report.json`; compute the gate decision; derive SARIF (import `formatSarif`); apply the `severity` filter then the `max-findings` cap (lowest-severity-first, log dropped count, keep ≤ `max-findings`); derive HTML via `format()`; derive the Markdown summary; write `report.sarif`, `scorecard.html`, and `$GITHUB_STEP_SUMMARY`.
