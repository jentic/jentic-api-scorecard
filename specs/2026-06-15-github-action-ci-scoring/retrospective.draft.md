...
- `plan.md` task 12 specified the README example as `uses: jentic/jentic-api-scorecard@v<major>`, implying a rolling major tag. The project tags concrete `vX.Y.Z` releases (lerna) with no maintained major alias, so the docs pin a concrete version / SHA instead.
