...
- `plan.md` Group 2 listed nine action inputs. Implementation added three more: `cli-version` (load-bearing for the CLI-version=image-tag invariant and for pinning the exports-carrying release), `artifact-name` (because `actions/upload-artifact@v4` rejects duplicate artifact names within a run, which the two-invocation self-test in validation.md §4 would otherwise hit), and `github-token` (defaults to the workflow token; lets advanced setups supply an App/PAT or a base-context `workflow_run` token to upload fork-PR findings the read-only default cannot).
