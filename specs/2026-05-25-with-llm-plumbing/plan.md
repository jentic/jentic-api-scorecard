...
4. Implement `detectLlmEnv(env: NodeJS.ProcessEnv): LlmEnvDetection` returning `{ forwardEnvVars: string[]; addHostFlag: boolean; hasUsableProvider: boolean }`. Iterate both allowlists; an env var is "present" when `env[name] !== undefined && env[name] !== ''`. `hasUsableProvider` is `true` when any cloud credential is present **or** when (`LLM_PROVIDER === 'OPENAI'` and `OPENAI_API_URL` is present and non-default).
...
14. When `options.withLlm` is unset, pass `forwardEnvVars: []` and `addHostFlag: false`. The existing `--with-llm` flag continues to be appended to the container's argv (current line 30–32).

## Group 4 — Docs and roadmap completion

15. Edit `docs/architecture.md` §5: add a "Bring your own LLM" subsection covering (a) the cloud recipe (existing — minor wording refresh to mention the env-var allowlist explicitly), (b) the local recipe (`LLM_PROVIDER=OPENAI`, `OPENAI_API_URL`, `OPENAI_API_KEY=<placeholder>`, `LLM_MODEL`), (c) host-network reachability (CLI auto-injects `--add-host=host.docker.internal:host-gateway` when a host-loopback URL is detected), (d) the security note (credentials visible in `docker inspect` for the run's lifetime — standard Docker behavior).
...
24. `npm run typescript:check-types -w @jentic/api-scorecard-cli` exits 0 (or whatever the `lerna run typescript:check-types` invocation that mirrors the PostToolUse hook is — confirm during implementation).
