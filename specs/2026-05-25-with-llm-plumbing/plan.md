...
12. When `options.withLlm && !detection.hasUsableProvider`, write a multi-line guidance message to `process.stderr.write(...)` and `return ExitCode.GENERIC_ERROR`. Guidance names both recipes — cloud (one of `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `GEMINI_API_KEY` / `AWS_ACCESS_KEY_ID` **plus** the matching `LLM_PROVIDER` value, since the engine defaults `LLM_PROVIDER=BEDROCK`) and local (`LLM_PROVIDER=OPENAI`, `OPENAI_API_URL=http://localhost:11434/v1/chat/completions`, `OPENAI_API_KEY=ollama`, `LLM_MODEL=<your-model>`). The guidance does **not** contain any captured env values; only the variable names.
