...
- **Exit-code contract is public.** The CLI must map server responses back to the same exit codes the one-shot path produces (0/2/3/6/7, host 4), so automation sees identical behavior regardless of mode.
