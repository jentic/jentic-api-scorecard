...
- **`jentic-openapi-common`** — the engine's canonical OpenAPI/Swagger version detector (`jentic.apitools.openapi.common.version_detection`: `get_version` / `is_openapi_32`). The runner's `version_guard.py` delegates version *detection* to it (keeping only the 3.2-refusal *policy* in-repo), so there is one source of truth for parsing version strings. It already arrives transitively through the engine; we pin it directly in `docker/pyproject.toml` so the import is honest. Pinned exactly, on the SpecLynx-family version line (`jentic-openapi-*`), which moves independently of `jentic-apitools-*` — keep both lines in sync when bumping the engine. See `docs/architecture.md` §6 and issue #113.
