...
- **`@tanstack/intent` (agent-skill distribution)** — dev-only tooling for shipping a versioned agent skill alongside the npm package. The canonical skill lives at the repo-root `skills/jentic-api-scorecard/SKILL.md` (with `references/` for progressive disclosure), grounded in the real CLI surface. Two distribution paths share the same `skills/<name>/SKILL.md` + `name`/`description`-frontmatter convention, so a single source serves both: TanStack Intent discovers the skill from the installed package (recognized via the `tanstack-intent` keyword + `skills/` `files` entry on `packages/cli/package.json`), and the Vercel `skills` CLI (`npx skills add …`) reads it straight from the repo — the latter needs no dependency. The skill is copied into `packages/cli/` at `prepack` and removed at `postpack` (same `copyfiles` pattern as `LICENSE`/`NOTICE`/`README.md`), so git keeps one copy while the tarball carries a real one. CI gates it via `skills:validate` (`intent validate`) and `skills:install:test` (`scripts/assert-skill-packaged.mjs` asserts the tarball includes the skill). Pinned exactly (`0.0.42`) because Intent is pre-1.0 alpha. The convention — not the tool — is load-bearing; the Vercel CLI would still work if Intent were dropped. See `docs/architecture.md` §4 layout notes.
