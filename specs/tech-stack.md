...
- **Output formats and CLI surface.** `--format` (`pretty` + `json`), `--detail`, and `-o / --output` are shipped. `--quiet`, `--verbose` are deferred to Phases 9 / 7. The Markdown formatter and `--format html` (Phase 14) remain deferred.
